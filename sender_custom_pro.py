...
def worker(port, data):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
        ...
        send_closing_message(seq_id, udp_socket)
...
for i in range(2,2 + SEGMENT_NUMBER):
    port = 5000 + i
    print("port:", port)

    ending_index = int(current_index + segment_length)

    print("Length of data:", len(data))
    print("Segment length:", segment_length)
    print("Current index:", current_index)
    print("Segment ends at:", ending_index)

    temp_data = data[int(current_index):ending_index]
    current_index += segment_length

    thread = threading.Thread(target=worker, args=[port, temp_data])
    thread.start()
